...
class SegmentDisplay {
  ...
  initSegments() {
	...
	// Returns the corresponding ASCII codes of the digits four choices.
	get SevenSegCodes() {
 ...
 // Shift right transformation  used for half horionztal segments
	shiftRightTransform(){
  ...
  getTransformationsSeven() {
	...
	return [
	  ...
	  function() {
		rotate(HALF_PI); //G 90 degree turn 
		translate(_this._segmentWidth, _this._segmentWidth);
	  },
	  function() {
		  rotate(HALF_PI); //Decimal point 
		  translate(_this._segmentLength*1.3, -(_this._segmentLength*1.6));
		  //translate(0, 0);
		  },
	];
  }
  ...
  getTransformationsFourteen() {
	...
	return [
	  ...
	  function() {_this.topDownTransform()}, //F
	  function() {
		rotate(HALF_PI); // G1, 90 degrees
		translate(_this._segmentWidth, _this._segmentWidth);
		},
	  function() {_this.shiftRightTransform();}, //G2
	  function() {
		rotate(PI*1.375); //  H , 78.7 degree
		translate((_this._segmentLength/2.2), -(_this._segmentLength/5));
		},
	  function() {
		rotate(PI*0.12); // J ,21.6 degree
		translate( -(_this._segmentLength/7), _this._segmentWidth+1 );
		},
	  function(){
		rotate(PI*0.12); // K , 21.6 degree
		translate(_this._segmentWidth, _this._segmentLength/6);
		},
	  function() {
		 rotate(PI); // L , 180 degree 
		 translate((_this._segmentLength/1.3333), _this._segmentLength/5.5);
		},
	  function() {
		 rotate(-(PI*0.12)); // M - 21.6 degree
		 translate(-(_this._segmentLength/9.5), -(_this._segmentLength/4));
		},
	  function() {
		 rotate(-(PI*0.12)); // N 21.6 degree
		 translate(_this._segmentWidth, -12);
		},
		function(){
  ...
  getTransformationsSixteen() {
	...
	return [
		...
		function() {
			rotate(PI / 2); //C  , degrees90 
			translate(_this._segmentWidth, - (((_this._segmentLength-_this._segmentWidth)/2)+_this._segmentWidth)); //C
		},
		...
		function() {
			rotate(HALF_PI); //G
			translate(_this._segmentWidth, - (((_this._segmentLength-_this._segmentWidth)/2)+_this._segmentWidth));
			},
		...
		function() {
			rotate(PI*0.12); // M ,21.6 degree
			translate( -(_this._segmentLength/16), _this._segmentWidth*1.1);  }, 
		function() { 
			rotate(PI*0.12); // N
			translate((_this._segmentLength/8), _this._segmentWidth);
			}, 
		function() {
			rotate(PI/2.6) //P
			translate(-(_this._segmentWidth/2), (_this._segmentWidth*1.2));
			}, 
		function() { 
			rotate(PI/2.6) //R
		...
		function() {rotate(PI*0.12); //T
			translate((_this._segmentLength/8), _this._segmentWidth)
			}, 
		function() {rotate(PI/2.6) //U
			translate(-(_this._segmentWidth/2), (_this._segmentWidth*1.2));
			}, 
		function() { // Dp
